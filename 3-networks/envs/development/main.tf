...
locals {
  ...
  base_subnet_primary_ranges = {
    ...
    /// add addresss range for region3
  }
...
module "base_shared_vpc" {
  ...
  default_region1               = var.default_region1
  default_region2               = var.default_region2
  //default_region3               = var.default_region3
  domain                        = var.domain
  bgp_asn_subnet                = local.bgp_asn_number
  windows_activation_enabled    = var.windows_activation_enabled
  dns_enable_inbound_forwarding = var.dns_enable_inbound_forwarding
  ...
  subnets = [
    ...
    {
      subnet_name           = "sb-${local.environment_code}-shared-base-${var.default_region3}"
      subnet_ip             = local.base_subnet_primary_ranges[var.default_region3]
      subnet_region         = var.default_region3
      subnet_private_access = "true"
      subnet_flow_logs      = var.subnetworks_enable_logging
      description           = "Second ${local.env} subnet example."
    }
